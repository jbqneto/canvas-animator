...
export const AiImageModal: React.FC<AiImageModalProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Falha na geração de imagem.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-neutral-900 border border-neutral-700 rounded-xl shadow-2xl w-full max-w-xl overflow-hidden text-neutral-100 animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm text-white">
                Criar & Editar Imagens com Gemini
              </h3>
              <p className="text-[11px] text-neutral-400">
                Gere ilustrações, props e cenários para a animação
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex bg-neutral-950 p-1 rounded-lg border border-neutral-800">
            ...
            >
              <Wand2 size={14} />
              Editar Frame Atual do Canvas
            </button>
          </div>

          {/* Aspect Ratio */}
          <div className="flex items-center justify-between text-xs">
            <span className="text-neutral-400">Formato / Proporção:</span>
          ...
          <div>
            <label className="text-xs text-neutral-300 block mb-1 font-medium">
              {mode === 'create'
                ? 'Prompt descritivo da imagem:'
                : 'Instruções de edição para o frame atual:'}
            </label>
            <textarea
              rows={3}
              value={prompt}
              ...
              placeholder={
                mode === 'create'
                  ? 'Ex: Cenário futurista de sala de aula com hologramas de matemática e estilo 2D limpo...'
                  : 'Ex: Transforme o desenho em estilo neon cyberpunk mantendo a pose do palito...'
              }
          ...
          >
            {isLoading ? (
              ...
              <>
                <Sparkles size={16} />
                {mode === 'create' ? 'Gerar Imagem' : 'Aplicar Edição com IA'}
              </>
            )}
          </button>
          ...
          {generatedImage && (
            <div className="pt-2 border-t border-neutral-800 space-y-3">
              <span className="text-xs text-emerald-400 font-semibold flex items-center gap-1">
                <Check size={14} /> Imagem gerada com sucesso!
              </span>
