...
const ALLOW = [/^FlashMotion/, /^Flash \+ Remotion$/, /^[\d.,:%×x ()+-]+$/, /^F\d/, /^[A-Z0-9 _:.+\-×/()%]+$/, /^(Ctrl|Shift|Alt)\b/, /^https?:/, /^data:/, /^(video|image|application)\//, /monospace|sans-serif/, /^rgba?\(/, /^#[0-9a-f]{3,8}$/i, /^gemini-/, /^\d\.\d (Flash|Pro)/, /^Studio$/, /^s ·$/, /^[\s◆➔→FP0-9]+$/, /^(Polygon|MultiPolygon|Feature|FeatureCollection|Sphere)$/, /^\s*<svg/, /^(linear|radial)-gradient\(/, /^((left|right|center|top|bottom)( |, |$))+$/];
